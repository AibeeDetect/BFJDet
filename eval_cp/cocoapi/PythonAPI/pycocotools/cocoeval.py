...
class COCOeval:
    ...
    def evaluateImg(self, imgId, catId, aRng, maxDet):
        ...
        # dt = [i for i in dt if i['score'] > 0.3]
    ...
    def summarize(self):
        ...
        def _summarize( ap=1, iouThr=None, areaRng='all', maxDets=100 ):
            ...
            typeStr_body = '(AP_body)' if ap==1 else '(AR_body)'
            typeStr_face = '(AP_face)' if ap == 1 else '(AR_face)'
            ...
            if len(s[s>-1])==0:
                ...
                if len(s.shape) == 3:
                    for i in range(s.shape[1]):
                        res = np.mean(s[:, i][s[:, i] > -1])
                        if i == 0:
                            print(iStr.format(titleStr, typeStr_body, iouStr, areaRng, maxDets, res))
                        else:
                            print(iStr.format(titleStr, typeStr_face, iouStr, areaRng, maxDets, res))
                else:
                    for i in range(s.shape[2]):
                        ...
                        if i == 0:
                            ...
                            print(iStr.format(titleStr, typeStr_face, iouStr, areaRng, maxDets, res))
                    # print(np.mean(s[:,:, 0][s[:,:, 0] > -1]))
                    # print(np.mean(s[:,:, 1][s[:,:, 1] > -1]))
                # print(s)
